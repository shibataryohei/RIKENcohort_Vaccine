...
map(Variable_SA, function(x){
  ...
  do.call(bind_rows, .) -> SA_Hib_LR_tbw
...
```{R}
Hib_LR_SA_tbw %>% 
  select(-Sibling, -`HBM@1M`, -Sex) %>% 
  setNames(c("SubjectID", "Nasopharynx", "Skin", "Any site",
             "Group", "Feces")) %>% 
  c2f -> group_tbw
  

SA_Hib_LR_tbw %>% 
  mutate(term = gsub(": Staphylococcus aureus@1M`1|`", "", term)) %>% 
  mutate(term = gsub("Staphylococcus aureus@1M1", "Any site", term)) %>% 
  mutate(term = fct_relevel(term, "Skin", "Nasopharynx", "Feces")) %>% 
  arrange(term) %>% 
    mutate(Model = fct_recode(Model,
                            "Adjusted‡" = "Adjusted")) %>% 
    mutate(Model = fct_relevel(Model, "Unadjusted")) -> res_tbw
  
  format_data_forestplot(group_data = group_tbw,
                         res_data = res_tbw,
                         outcome = "Group",
                         group = "Phenotype",
                         group_reference = Phenotype_Reference,
                         colname_model = "Model",
                         colname_evalue = "E value")
```
                         
                         
```{R}
forestplot %>%
  mutate(Phenotype = if_else(Phenotype == "",
                             glue(""),
                             glue("Phenotype {Phenotype}"))) %>% 
  .$Phenotype %>% 
  as.character %>% 
  map(., ~ bquote(expression(bold(.(.x))))) %>% 
  c(list("", 
         expression(bold("Phenotype"))),
    .) -> phenotype_list

forestplot %>% 
  .$`Number of the outcome, n (%)` %>% 
  as.character %>% 
  map(., paste) %>% 
  c(list(expression(bold("Number of")),
         expression(bold("the outcome, n (%)"))),
    .) -> number_outcome_list
...
forestplot %>% 
  ...
  as.character %>% 
  map(., paste) %>% 
  c(list(expression(bold("Profile")),
         expression(bold("size, n"))),
    .) -> number_total_list

forestplot %>% 
  .$`Odds ratio (95% CI)` %>% 
  as.character %>% 
  c(list(expression(bold("Odds ratio")),
...
forestplot %>% 
  ...
  map(., paste) %>% 
  c(list("", 
         expression(bold("Model"))),
    .) -> model_list

model_list[c(6, 9, 12)] <- list(expression("Adjusted"^"†"))

forestplot %>% 
  .$`p.value` %>% 
  as.character %>% 
  c(list("",
         expression(bold("P-value"))),
    .) -> pvalue_list
