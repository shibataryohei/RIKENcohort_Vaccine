...
future_map(Exposures_SkinNP, function(x){
...
LOCOM_Pglobal_tbl %>% 
  rename(`P value` = P.global,
         Variable = Age) %>% 
  mutate(Variable = paste("    Global level at", Variable)) %>% 
  mutate(`Odds ratio (95% CI)` = NA,
         estimate = NA,
         conf.low = NA,
         conf.high = NA) %>% 
  mutate(`P value` = format_pvalue(`P value`)) %>% 
  mutate(`P value` = paste0(`P value`, "")) -> LOCOM_Pglobal_tbw

bind_rows(blank_tbw %>% 
            mutate(Variable = "Skin"),
          forest_tbw[1:2, ] %>% 
            mutate(Variable = gsub("Skin: ", "    ", Variable)),
          blank_tbw %>% 
            mutate(Variable = "Nasopharynx*"),
          forest_tbw[3:4, ] %>% 
            mutate(Variable = gsub("Nasopharynx: ", "    ", Variable)),
          blank_tbw %>% 
            mutate(Variable = "Feces"),
          LOCOM_Pglobal_tbw) -> forest2_tbw
```

```{R}
list(Variable = c(list(expression(bold("Variable"))),
                     forest2_tbw$Variable),
     `Odds ratio (95% CI)` = c(list(expression(bold("Odds ratio (95% CI)"))),
                               forest2_tbw$`Odds ratio (95% CI)`),
     `P value` = c(list(expression(bold("P value"))),
                   forest2_tbw$`P value`)) -> list
```
...
tiff("/Users/shibataryohei/Dropbox/Manuscript/RIKENcohort-Vaccine/Figure/Figure3.tiff",
      ...
      res = 600)
forestplot::forestplot(labeltext = list,
           xticks =  c(log(0.1), 0, log(10), log(20)), # range of x axis
           xticks.digits = 0,
           ...
           ci.vertices = TRUE,
           ci.vertices.height = 0.2 ,
           mean = c(NA,  forest2_tbw$estimate),
           lower = c(NA, forest2_tbw$conf.low),
           upper = c(NA, forest2_tbw$conf.high))
dev.off()
```
![](/Users/shibataryohei/Dropbox/Manuscript/RIKENcohort-Vaccine/Figure/Figure2.tiff)
