...
Metadata_tbw %>% 
  ...
  ungroup %>% 
  select(-`Daycare_Ever@6M`, -`HBM@6M`, -`HBM@9M`) %>% 
  mutate(`7Y` = if_else(`7Y` == "Low",
                        1, 0)) -> Metadata_Hib_tbw

summary(Metadata_Hib_tbw)
```

```{R}
Metadata_Hib_tbw %>% 
  names %>% 
  .[-1:-2] -> Exposures_Metadata
...
future_map(Exposures_Metadata, function(x){
  ...
  do.call(bind_rows, .) %>% 
  tidy_table() %>% 
  table2png("Aim2/Table_Clinicaldata_Hib.png")
```
...
```
![Logistic regression model.](Aim4/Table_Colonization_LR.png)
